...
export const NamesPatternChart = ({ 
  ...
  const chartData = Object.entries(frequency)
    .map(([names, count]) => ({
      names: Number(names),
      count: count,
      isHighlighted: count > 3,
    }))
    .sort((a, b) => a.names - b.names)
    .slice(0, 20); // Show top 20 values

  const mean = namesPattern.length > 0 
    ? namesPattern.reduce((sum, n) => sum + n, 0) / namesPattern.length 
    : 0;
  
  const variance = namesPattern.length > 0
    ? namesPattern.reduce((sum, n) => sum + Math.pow(n - mean, 2), 0) / namesPattern.length
    : 0;
          ...
          {mostCommonCount && (
            <div className="bg-muted p-3 rounded-md">
              ...
              <div className="text-sm text-muted-foreground">
                ...
                {mostCommonFrequency > 3 && (
                  <span className="text-orange-600 ml-2">(⚠ Suspicious)</span>
                )}
              </div>
            </div>
          )}
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <XAxis 
                  dataKey="names" 
                  label={{ value: 'Total Names', position: 'insideBottom', offset: -5 }}
                />
