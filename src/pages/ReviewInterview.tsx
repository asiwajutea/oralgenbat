...
const ReviewInterview = () => {
  ...
  const { data: audit, isLoading: auditLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("audits").select("*").eq("id", auditId).maybeSingle();
  ...
  const {
    ...
    isFetching: isFieldAuditFetching,
  } = useQuery({
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-4">
            This interview is currently being reviewed by another auditor. Please try again later or select a different
            interview.
          </p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center max-w-md p-8">
          ...
          <h2 className="text-2xl font-bold mb-2">Interview {isPassed ? "Passed" : "Failed"}</h2>
          ...
          <p className="text-muted-foreground mb-1">Your review has been submitted successfully.</p>
          ...
          {hasNextInterview ? (
            <div className="mt-4 mb-6">
              <p className="text-sm text-muted-foreground">
                Next interview:{" "}
                <span className="font-mono font-medium text-foreground">{nextAudit.file_name || nextAudit.id}</span>
              </p>
              <div className="flex items-center justify-center gap-2 mt-2">
                <Clock className="h-4 w-4 text-primary animate-pulse" />
                <span className="text-sm font-medium text-primary">Auto-loading in {countdown}s...</span>
              </div>
            </div>
          ) : (
            <p className="text-sm text-muted-foreground mt-4 mb-6">No more interviews to review.</p>
          )}
          <div className="flex gap-3 justify-center mt-2">
            ...
            <Button
              variant={hasNextInterview ? "outline" : "default"}
              onClick={() => {
                cancelCountdown();
                navigate("/interviews");
              }}
  ...
  return (
    <>
      <div className="h-screen flex flex-col lg:flex-row">
        ...
        <div
          className={`w-full lg:w-1/2 border-r border-border bg-background h-[calc(100vh-49px)] lg:h-screen flex flex-col ${
          ...
          <div className="flex-shrink-0 p-3 sm:p-4 pb-3 border-b border-border">
            ...
            <div className="mt-3 sm:mt-4">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                <p className="text-xs text-muted-foreground font-medium truncate">{audit.file_name}</p>
                {burnHistory?.get(audit.id) && <BurnHistoryIcon entry={burnHistory.get(audit.id)} />}
                {isAuditor && (
                ...
                {fieldAuditData?.found ? (
                  <Badge className="bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] px-1.5 py-0 gap-1 flex-shrink-0">
                    ...
                    Field Audited
                    {fieldAuditData.reviewed_at
                      ? ` - ${format(new Date(fieldAuditData.reviewed_at), "MMM d, yyyy")}`
                      : ""}
                  </Badge>
                  ...
                  <span className="flex items-center gap-1 flex-shrink-0">
                    <Badge
                      ...
                      className={`text-[10px] px-1.5 py-0 gap-1 ${
                        fieldAuditData.reason === "external_auth_error" ||
                        fieldAuditData.reason === "external_config_error" ||
                        fieldAuditData.reason === "external_error"
                    ...
                    >
                      <RefreshCw
                        className={`h-3 w-3 text-muted-foreground ${isFieldAuditFetching ? "animate-spin" : ""}`}
                      />
                    </button>
                  </span>
                ) : null}
          ...
          <div className="flex-shrink-0 sticky top-0 z-10 bg-background border-b border-border shadow-sm">
            ...
            {isAuditor && !isReviewed && metadata && (
              <div className="p-3 sm:p-4 space-y-3" ref={checklistRef}>
                ...
                <AuditChecklist
                  ...
                  isSticky={isSticky}
                  autoFlagged={!!fraudFlag?.is_flagged}
                  fraudCollisionCount={fraudFlag?.collisions?.length ?? 0}
            ...
            <ReviewActions
              ...
              audioAnalysisComplete={
                !!(metadata?.family_story_duration !== undefined && metadata?.family_story_duration !== null)
              }
              pdfAnalysisComplete={
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  ...
                  <span className="text-muted-foreground">
                    ...
                    {audit.review_duration_seconds && (
                      <span className="text-muted-foreground">
                        {" "}
                        ({formatReviewDuration(audit.review_duration_seconds)})
                      </span>
                    )}
                  </span>
                </div>
            ...
            {metadata ? (
              <>
                ...
                {metadata.pdf_clarity_score !== null ||
                metadata.pdf_handwriting_legibility !== null ||
                aiUnavailable ||
                !pdfAiEnabled ? (
                  <PDFAnalysisPanel
                    metadata={metadata}
        ...
        <div
          className={`w-full lg:w-1/2 h-[calc(100vh-49px)] lg:h-screen overflow-hidden bg-muted/5 flex flex-col ${
              ...
              <AlertDialog open={showDeletePdfDialog} onOpenChange={setShowDeletePdfDialog}>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will permanently delete the uploaded PDF for "{audit.file_name}". This action cannot be
                      undone.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
