...
const ReviewInterview = () => {
  ...
  const { data: metadata, isLoading: metadataLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("interview_metadata").select("*").eq("audit_id", auditId).single();
      if (error) throw error;
      return data;
    },
    enabled: !!auditId
  ...
  const { data: photos, isLoading: photosLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("interview_photos").select("*").eq("audit_id", auditId).order("display_order", { ascending: true });
      if (error) throw error;
      return data;
    },
    enabled: !!auditId
  ...
  const { data: nextAudit } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("audits").select("id").or('status.in.(Pending,Awaiting Review),reviewed_by.is.null').neq("id", auditId).order("uploaded_at", { ascending: true }).limit(1).maybeSingle();
      if (error) throw error;
      return data;
    },
    enabled: !!auditId
  ...
  const { data: checklistProgress, isLoading: checklistLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("audit_checklist_progress").select("*").eq("audit_id", auditId).maybeSingle();
      if (error) throw error;
      if (!data) return null;
      ...
      <div className="w-1/2 border-r border-border bg-background h-screen flex flex-col">
        ...
        <div className="flex-shrink-0 sticky top-0 z-10 bg-background border-b border-border shadow-sm">
          ...
          <ReviewActions
            ...
            checklistCompleted={checklistCompleted || isReviewed}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {metadata ? (
            <>
              ...
              {metadata.family_story_audio_url && metadata.pedigree_segment_audio_url && !metadata.duration_manually_confirmed ? (
                <AudioPlayerPanel
                  auditId={auditId!}
