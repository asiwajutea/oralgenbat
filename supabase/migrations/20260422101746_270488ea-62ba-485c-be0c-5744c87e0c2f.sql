...
BEGIN
  v_is_privileged := has_role(auth.uid(), 'admin'::app_role)
  ...
  WITH scoped_audits AS (
    ...
    WHERE a.uploaded_at >= p_start_date
      ...
      AND (v_is_privileged OR public.user_can_view_audit_for_tracking(a.id))
  ),
  -- All completed checklist runs for those audits (used for error counts)
  completed_runs AS (
    SELECT
      acp.audit_id,
      acp.items,
  ...
  -- Earliest completed checklist per audit (first-audit metric)
  first_runs AS (
