...
BEGIN
  ...
  -- Pending (includes Awaiting Review non-re-audit)
  SELECT 'Pending'::text, count(*)::bigint, coalesce(sum(b.names),0)::bigint
  FROM base b
  WHERE b.status IN ('Pending', 'Awaiting Review')
    AND NOT (coalesce(b.is_re_audit, false) = true AND b.status = 'Awaiting Review')
  ...
  FROM base b
  WHERE coalesce(b.is_re_audit, false) = true AND b.status = 'Awaiting Review'
