...
const SENDER_EMAIL = "Zamoph.audit@gmail.com";
const REPLY_TO = "Zamoph.audit@gmail.com";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!res.ok) {
      return new Response(JSON.stringify({ error: `Gmail send failed [${res.status}]`, details: data }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
